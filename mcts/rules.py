...
from utils import Deck, Card, Trash, Color
...
class Rules:
    ...
    @staticmethod
    def get_rules_moves(state: GameState, player: int) -> List[Action]:
        """
        ...
        return list(set([m for m in moves if m is not None]))
    ...
    def _get_probabilities(
        ...
        Args:
            ...
            trash: the trash of the game
        """
        probabilities = np.empty(len(hand), dtype=float)
    ...
    # RULE 1
    @staticmethod
    def _tell_most_information() -> Optional[Action]:
        """
        ...
        while True:
            destination = Rules._state.get_next_player(destination)
    ...
    @staticmethod
    def _tell_risky_card() -> Optional[Action]:
        pass

    # RULE 7
    @staticmethod
    def _play_probably_safe(threshold: float = 0.7) -> Optional[Action]:
        """
        ...
        if np.max(probabilities) >= threshold:
            return Action(
                player=Rules._player, action_type=action_type, card_idx=np.argmax(probabilities)
            )
        else:
            return None
    ...
    # RULE 8
    @staticmethod
    def _play_probably_safe_late(threshold: float = 0.4) -> Optional[Action]:
        """
    ...
    # RULE 9
    @staticmethod
    def _discard_probably_useless(threshold: float) -> Optional[Action]:
        """
        ...
        if np.max(probabilities) >= threshold:
            ...
            return move
        elif Rules._state.hints >= RULE_9_MIN_HINTS:
        ...
        return Action(player=Rules._player,action_type= action_type, card_idx=best_idx)

    # RULE 10
    @staticmethod
    def _discard_least_likely_to_be_necessary(threshold: int) -> Optional[Action]:
        """
