...
def format_action(xxx_todo_changeme12, gid, replay=None):
    (i,(action,pnr,card)) = xxx_todo_changeme12
    result = "You "
    other = "the AI"
    otherp = "their"
    if pnr == 0:
...
def show_game_state(game, player, turn, gid, replay=False):
    ...
    def make_ai_card(xxx_todo_changeme10, highlight):
    
        (i,(col,num)) = xxx_todo_changeme10
    ...
    while len(aicards) < 10:
        aicards.append("")
    def make_your_card(xxx_todo_changeme11, highlight):
        (i,(col,num)) = xxx_todo_changeme11
...
def make_card_image(xxx_todo_changeme13, links=[], highlight=False):
    (col,num) = xxx_todo_changeme13
    image = """
...
"""
    ...
    circ = "\n".join([make_circle(x_y[0],x_y[1],hanabi.COLORNAMES[col]) for x_y in circles[num]])
    highlighttext = ""
    if highlight:
...
class MyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def perform_response(s):
        ...
        if path.startswith("/tutorial"):
            ...
            s.wfile.write("<html><head><title>Hanabi</title></head>".encode())
            s.wfile.write('<body><center>'.encode())
            ...
            s.wfile.write('</center></body></html>'.encode())
            return
        ...
        if path.startswith("/new/study/"):
            ...
            s.wfile.write('<h1>Replay selection</h1>'.encode())
            s.wfile.write('<h2>Filters:</h2>'.encode())
            ...
            s.wfile.write('<p>If you continue, you will take over playing the game you were viewing from turn %d onward</p>'%round.encode())
            s.wfile.write('<p>You may choose to use the same AI as the player that was playing the game by clicking <a href="/starttakeover/%s/%d/%s/%s/%s">here</a></p>\n'%(gid,round+1,ai,action,arg).encode())
            s.wfile.write('<p>You may also choose any AI to play with:</p><ul>'.encode())
            for a in ais:
                s.wfile.write('<li><a href="/starttakeover/%s/%d/%s/%s/%s">%s AI</a></li>'%(gid,round+1,a,action,arg,a.capitalize()).encode())
                
            s.wfile.write("</ul></p></body></html>".encode())
            return
        ...
        if gid is None or game is None or path.startswith("/restart/"):
            ...
            s.wfile.write('</ul><br/>'.encode())
            s.wfile.write('<p>Or select a <a href="/selectreplay/">replay file to view</a></p>'.encode())
            s.wfile.write('</body></html>'.encode())
            return
    ...
    def show_explanation(s, game):
        s.wfile.write("<html><head><title>Hanabi - AI Explanation</title></head>".encode())
        s.wfile.write('<body>'.encode())
        
        s.wfile.write('<table border="1">'.encode())
    ...
    def presurvey(s, gid, warn=False):
        s.wfile.write('<center><h1>Finally, please answer some question about previous board game experience</h1>'.encode())
    ...
    def postsurvey(s, gid, warn=False):
        s.wfile.write('<center><h1>Please answer some questions about your experience with the AI</h1>'.encode())
        s.wfile.write('<table width="600px">\n<tr><td>'.encode())
    ...
    def do_POST(s):
        ...
        if s.path.startswith("/submitpost2"):
            ...
            s.wfile.write("<html><head><title>Hanabi</title></head>".encode())
            s.wfile.write('<body><center>'.encode())
            s.wfile.write("<h1>Thank you for your participation in this study!</h1>".encode())
            
            s.wfile.write('<a href="/new/study/%s">Play again</a><br/>You can bookmark this link and play again at any later time. <br/>Any additional games you play will also be recorded for future analysis, and published if you agreed to publication.'%(gid).encode())
            ...
            if gid not in participants:
                s.wfile.write("<html><head><title>Hanabi</title></head>".encode())
                s.wfile.write('<body><center>'.encode())
                s.wfile.write("<h1>Session timed out. Thank you for your time and participation</h1>".encode())                
                s.wfile.write('</body></html>'.encode())
                return
            participantslock.acquire()
            for r in required:
    ...
    def consentform(s):
        ...
        s.wfile.write('<center><font size="12"><a href="/tutorial">By clicking here I agree to participate in the study</a></font></center><br/><br/><br/>'.encode())
        s.wfile.write("</body></html>".encode())
